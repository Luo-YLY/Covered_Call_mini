...
最终交付包的冻结数据范围固定为原始五只ETF：`159915、510050、510300、510500、588000`。打包时会从标准化行情与Delta派生表中只保留这五只ETF，并排除工作区内的上传样本、用户提交数据及动态验收结果。新增ETF能力仍完整保留；交付后可在“数据与回测”页面提交新的三张CSV，不会改变初始冻结数据边界。
